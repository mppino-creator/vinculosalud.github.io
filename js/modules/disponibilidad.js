...
// ============================================
// FUNCIONES DE DISPONIBILIDAD (PSICÓLOGOS)
// ============================================
...
export function toggleWeekday(day) {
    ...
    if (state.selectedWeekdays.includes(day)) {
        state.selectedWeekdays = state.selectedWeekdays.filter(d => d !== day);
        element.classList.remove('selected');
    } else {
        state.selectedWeekdays.push(day);
...
function toggleGeneratedSlot(time) {
    ...
    if (index !== -1) {
        state.generatedSlots.splice(index, 1);
    } else {
        state.generatedSlots.push({ time: time, isOvercupo: false });
    }
    renderGeneratedSlots();
}
...
export function blockTimeRange() {
    ...
    state.generatedSlots = state.generatedSlots.filter(slot => {
...
function toggleTimeSlot(time, isOvercupo = false) {
    ...
    if (index === -1) {
        state.currentUser.data.availability[date].push({ time: time, isOvercupo: isOvercupo });
    } else {
