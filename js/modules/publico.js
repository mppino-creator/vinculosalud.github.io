...
export function showSection(sectionId) {
    ...
    // 1. DEFINIR TODAS LAS SECCIONES DISPONIBLES (AGREGAR instagramSection)
    const secciones = ['inicio', 'about', 'equipo', 'atencion', 'contacto', 'instagramSection'];
    ...
    if (seccionAMostrar) {
        ...
        const yOffset = -100;
        const y = seccionAMostrar.getBoundingClientRect().top + window.pageYOffset + yOffset;
        window.scrollTo({ top: y, behavior: 'smooth' });
    }
    
    // 4. ACTUALIZAR CLASES ACTIVAS EN EL MENÚ (VERSIÓN MEJORADA)
    document.querySelectorAll('nav.glass a, .public-nav a').forEach(link => {
        ...
        if (link.getAttribute('onclick')?.includes(sectionId)) {
            ...
            link.style.borderBottom = '2px solid #C8A161';
...
// ============================================
// FUNCIÓN PARA ACTUALIZAR SECCIÓN INSTAGRAM (NUEVA)
// ============================================
export function updateInstagramSection() {
    console.log('📸 Actualizando sección de Instagram...');
    
    const instagramSection = document.getElementById('instagramSection');
    if (!instagramSection) return;
    
    // Usar los datos del state o valores por defecto
    const instaData = window.state?.instagramData || {
    ...
    if (titleEl) titleEl.textContent = instaData.title;
    if (subtitleEl) subtitleEl.textContent = instaData.subtitle;
    if (quoteEl) quoteEl.innerHTML = instaData.quote;
    if (textEl) textEl.textContent = instaData.text;
    if (messageEl) messageEl.textContent = instaData.message;
    if (linkEl) linkEl.href = instaData.link;
    if (imageEl) imageEl.src = instaData.image;
}
// ============================================
// COMPARTIR PERFIL
// ============================================
...
// ============================================
// EXPORTACIÓN AL OBJETO WINDOW (ACTUALIZADO)
// ============================================
if (typeof window !== 'undefined') {
    ...
    // 🆕 NUEVO: Exportar función de Instagram
    window.updateInstagramSection = updateInstagramSection;
